...
export default {
  ...
  "whiteknight": {
    ...
    "userFlags": {
      "_comment": "Bit INDICES into Deployer.user_flags (u64) — the switches a USER owns, moved only by set_user_flags(mask, value) signed by the manager authority. An operator can never write them. The value is 1n << index, and 0 is always the pre-existing behaviour, which is why the field could be carved into live accounts without a migration. PAUSE_MINING stops deploys only: settles, claims, prize collection and withdrawals all keep running. HOLD_VAULT_BUYS stops BOTH ticket legs so hashrate banks on the Miner; it does not stop claim_sats, which is money owed to the user. A mask touching any bit not listed here is rejected with BadSetting.",
      "PAUSE_MINING": 0,
      "HOLD_VAULT_BUYS": 1
    },
    "userFlagsOffset": 205,
